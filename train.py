...
def _train(model, args):
    ...
    # from Dehazing.ITS.CR import ContrastLoss
    # Contrast = ContrastLoss(ablation=False).to(device)
    summary(model, (3, 256, 256))
    num_params = sum(param.numel() for param in model.parameters())
    num_params_in_millions = num_params / 1e6
    print(f'model parameters: {num_params_in_millions:.2f} M')
    ...
    iter_timer = Timer('m')
    best_psnr=-1
    with open('metrics.txt', 'w') as log_file:
       log_file.write('Epoch\tLoss\tPSNR\tSSIM\n')
       for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            torchvision.utils.save_image(pred_img[2],
                                         "output"
                                         "/" + str(iter_idx + 1) + ".jpg")
        ...
        if epoch_idx % args.valid_freq == 0:
            val1,val2 = _valid(model, args, epoch_idx)
            log_file.write(f'{epoch_idx}\t{loss:.4f}\t{val1:.4f}\t{val2:.4f}\n')
            log_file.flush()

            writer.add_scalar('Loss', loss, epoch_idx)
            writer.add_scalar('PSNR', val1, epoch_idx)
            writer.add_scalar('SSIM', val2, epoch_idx)

            print('%03d epoch \n Average PSNR %.2f dB' % (epoch_idx, val1))
            print('%03d epoch \n Average SSIM %.2f dB' % (epoch_idx, val2))
            writer.add_scalar('PSNR', val1, epoch_idx)
            if val1 >= best_psnr:
